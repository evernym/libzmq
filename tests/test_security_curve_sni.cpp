...
static char server1_public [41];
static char server1_secret [41];
static char server2_public [41];
static char server2_secret [41];
...
int main (void)
{
    ...
    assert (rc == 0);
    rc = zmq_curve_keypair (server1_public, server1_secret);
    assert (rc == 0);
    rc = zmq_curve_keypair (server2_public, server2_secret);
    assert (rc == 0);
